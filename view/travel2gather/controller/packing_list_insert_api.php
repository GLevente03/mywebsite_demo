...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if($receivedApiKey == $apiKey){
        ...
        $who_brings = $data["who_brings"];
        $added_by = $data["added_by"];

        $insertion = new DatabaseCommunication();

        try{
            $id = $insertion->insert("INSERT INTO packing_list (trip_id, item_name, who_brings, added_by) VALUES (?, ?, ?, ?)", $trip_id, $item_name, $who_brings, $added_by);
